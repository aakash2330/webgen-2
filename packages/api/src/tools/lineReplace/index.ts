...
const replaceSchema = z.object({
  ...
  search: z
    .string()
    .describe("Original content to match (supports ... as ellipsis)"),
...
export function getLineReplaceTool(projectId: string) {
  return {
    "webgen-line-replace": tool({
      description:
        "Replace a specific range in a file with validation using an ellipsis matcher.",
...
export async function lineReplace({
  ...
  if (
    !matchesWithEllipsis(lines, first_replaced_line, last_replaced_line, search)
  ) {
  ...
  const res = await putTextObject(key, nextContent);
  console.log({ res });
